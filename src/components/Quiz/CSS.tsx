  ...
  useEffect(
    function () {
      async function fetchData() {
        ...
        setQuestions(response.quizzes[1].questions[questionCount]); // grabs a single object from the response json
        setAnswer(response.quizzes[1].questions[questionCount].answer);
        // select answer string
        setIcon(response.quizzes[1].icon);
        setTitle(response.quizzes[1].title);
      }
      fetchData();
    },
    [questionCount]
  ...
  return (
    <div>
      <div className="flex items-center">
        <span className="bg-[var(--css-background)] p-1 rounded-lg">
