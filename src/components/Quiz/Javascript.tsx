  ...
  useEffect(
    function () {
      async function fetchData() {
        ...
        setQuestions(response.quizzes[2].questions[questionCount]); // grabs a single object from the response json
        setAnswer(response.quizzes[2].questions[questionCount].answer);
        // select answer string
        setIcon(response.quizzes[2].icon);
        setTitle(response.quizzes[2].title);
      }
      fetchData();
    },
    [questionCount]
  ...
  return (
    <div>
      <div className="flex items-center">
        <span className="bg-[var(--javascript-background)] p-1 rounded-lg">
          <img src="/images/icon-js.svg" alt="Javascript icon" />
        </span>
        <h1 className="ml-3">Javascript</h1>
      </div>
        ...
        <div className="flex flex-col gap-40 max-sm:flex-col-reverse max-sm:gap-5">
          ...
          <div>
            <progress
              value={progressBar}
              max={100}
              className="w-[80%] mx-auto max-sm:w-full overflow-hidden [&::-webkit-progress-bar]:bg-[var(--option-bg)] [&::-webkit-progress-value]:bg-[var(--submit-button)] [&::-moz-progress-bar]: bg-[var(--submit-button)] h-2 rounded-full"
            ></progress>
          </div>
        </div>
        <ul>
